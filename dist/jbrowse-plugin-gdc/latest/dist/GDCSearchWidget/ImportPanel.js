...
import { Alert, Paper, Button, Typography, TextField, Chip, } from '@mui/material';
...
const Panel = ({ model }) => {
    ...
    function determineFileInfo(fileName) {
        var _a;
        const format = (_a = fileName.split('.').at(-1)) !== null && _a !== void 0 ? _a : '';
        ...
        onDrop: async (acceptedFiles, rejectedFiles) => {
            ...
            if (rejectedFiles.length) {
                ...
                if (acceptedFiles.length || rejectedFiles.length > 1) {
                    ...
                    const message = `File size is too large (${Math.round(firstRejected.file.size / 1024 ** 2)} MiB), max size is ${MAX_FILE_SIZE / 1024 ** 2} MiB`;
                    console.error(message);
                    setDragErrorMessage(message);
                }
                else {
                    ...
                    if (fileInfo.format == 'json') {
                        const res = await new Promise(resolve => {
                            ...
                            reader.addEventListener('load', event => {
                                var _a;
                                resolve(JSON.parse((_a = event.target) === null || _a === void 0 ? void 0 : _a.result));
                            });
        ...
        React.createElement(Paper, { className: classes.paper, elevation: 3 },
            ...
            React.createElement("div", { className: classes.submitContainer },
                ...
                React.createElement("div", { className: classes.buttonContainer },
                    React.createElement(Button, { color: "primary", variant: "contained", size: "large", onClick: async () => {
                            resetErrorMessages();
                            try {
                                // @ts-expect-error
                                ...
                                if (!err.includes('unable to determine size of file at')) {
                                    console.error(e);
                                    const message = err.length > 100 ? `${err.slice(0, 99)}...` : err;
                                    setTrackErrorMessage(`Failed to add track.\n ${message}.`);
                                }
                            }
                            // @ts-expect-error
