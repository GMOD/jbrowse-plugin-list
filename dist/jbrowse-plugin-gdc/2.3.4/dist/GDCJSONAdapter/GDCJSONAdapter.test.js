...
import { mutationsJSON, genesJSON, gdcResponse, } from './test_data/json_test_data.js';
test('adapter can fetch features from a mutations explore json file', async () => {
    const adapter = new GDCJSONAdapter(configSchema.create({
...
test('adapter can fetch featues from a genes explore json file', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce(Response.json(gdcResponse)));
    const featuresArray = await firstValueFrom(features.pipe(toArray()));
    expect(featuresArray.slice(0, 3)).toMatchSnapshot();
});
//# sourceMappingURL=GDCJSONAdapter.test.js.map