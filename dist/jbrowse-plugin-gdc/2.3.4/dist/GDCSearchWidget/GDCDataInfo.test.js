...
test('maps data information to BAM', async () => {
    ...
    const result = mapDataInfo({ type: 'Aligned Reads', category: 'Sequencing Reads', format: 'bam' }, uri, indexId);
    expect(result).toEqual({
        config: {
...
test('maps gdc explore to config', async () => {
    ...
    const dateNowStub = vi.fn(() => 1530518207007);
    global.Date.now = dateNowStub;
    const result = mapGDCExploreConfig('GDC Explore', 'gene', 'test-filters', 'track-id');
    expect(result).toEqual({
        config: {
            adapter: {
                type: 'GDCAdapter',
...
//# sourceMappingURL=GDCDataInfo.test.js.map