...
function wrapSequence(segments, sequenceWrapLength, getContrastText) {
    ...
    for (const [index, segment] of segments.entries()) {
        ...
        const span = (jsx("span", { style: style, children: printLines.join('\n') }, `${segment.type}-${index}`));
...
function SequenceViewer({ locationIntervals, refSeqName, sequenceSegments, strand, }) {
    ...
    return (jsxs(Fragment, { children: [jsx(Button, { variant: "contained", onClick: onCopyClick, style: { marginLeft: 10 }, size: "medium", children: "Copy sequence" }), jsxs(Paper, { style: {
                    fontFamily: 'monospace',
                    ...
                    overflowX: 'auto',
                        ...
                        .join(';'), "(strand=", strand === 1 ? '+' : '-', ";length=", getSequenceLength(sequenceSegments), ")", jsx("br", {}), wrapSequence(sequenceSegments, SEQUENCE_WRAP_LENGTH, (color) => theme.palette.getContrastText(color))] })] }));
}
...
const ApolloFeatureDetailsWidget = observer(function ApolloFeatureDetailsWidget(props) {
    ...
    return (jsxs("div", { className: classes.root, children: [jsx(BasicInformation, { feature: feature, session: session, assembly: currentAssembly._id }), jsxs(Accordion, { style: { marginTop: 10 }, expanded: panelState.includes('attributes'), onChange: (e, expanded) => {
                ...
                }, children: [jsx(AccordionSummary, { expandIcon: jsx(ExpandMoreIcon, { style: { color: 'white' } }), "aria-controls": "panel2-content", id: "panel2-header", children: jsx(Typography, { component: "span", children: "Sequence" }) }), jsx(AccordionDetails, { children: panelState.includes('sequence') && sequence && (jsx(SequenceViewer, { refSeqName: refName, strand: feature.strand === -1 ? -1 : 1, locationIntervals: locationIntervals, sequenceSegments: sequenceSegments })) })] }), jsxs(Accordion, { style: { marginTop: 10 }, expanded: panelState.includes('related_features'), onChange: (e, expanded) => {
...
function SequenceOptionSelector({ onChange, options, value, }) {
    return (jsx(Select, { value: value, onChange: (e) => {
            onChange(e.target.value);
        }, size: "small", "data-testid": "sequenceOptionSelector", children: options.map((option) => (jsx(MenuItem, { value: option, "data-testid": `sequenceOption-${option}`, children: option }, option))) }));
}
...
const TranscriptSequence = observer(function TranscriptSequence({ assembly, feature, refName, session, }) {
    ...
    const sequenceSegments = useMemo(() => refData
    ...
    const locationIntervals = useMemo(() => getLocationIntervals(sequenceSegments), [sequenceSegments]);
    ...
    return (jsxs(Fragment, { children: [jsx(SequenceOptionSelector, { options: sequenceOptions, value: effectiveSelectedOption, onChange: setSelectedOption }), jsx(SequenceViewer, { refSeqName: refData.name, strand: feature.strand === -1 ? -1 : 1, locationIntervals: locationIntervals, sequenceSegments: sequenceSegments })] }));
});
