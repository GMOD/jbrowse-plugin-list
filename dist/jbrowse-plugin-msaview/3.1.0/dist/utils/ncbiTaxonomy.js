...
export async function resolveTaxId(query) {
    ...
    const json = await jsonfetch(eutilsUrl('esearch.fcgi', { db: 'taxonomy', term, retmax: '1' }));
 ...
 */
export async function resolveAssemblySpecies(assemblyName) {
    const term = assemblyName.trim();
    if (!term) {
        return undefined;
    }
    const search = await jsonfetch(eutilsUrl('esearch.fcgi', { db: 'assembly', term, retmax: '1' }));
    const uid = search.esearchresult?.idlist?.[0];
    if (!uid) {
        return undefined;
    }
    const summary = await jsonfetch(eutilsUrl('esummary.fcgi', { db: 'assembly', id: uid }));
