...
function makeModel() {
    ...
    const model = {
        ...
        setLaunchController(arg) {
            model.launchController = arg;
            writes.push(`setLaunchController:${arg ? 'set' : 'cleared'}`);
        },
        setProgress: (arg) => writes.push(`setProgress:${arg}`),
        setError: (e) => writes.push(`setError:${e instanceof Error ? e.message : String(e)}`),
        setRid: (arg) => writes.push(`setRid:${arg}`),
...
describe('a launch that is abandoned', () => {
    async function abandonMidFlight(reject) {
        const { model, writes } = makeModel();
        let released;
