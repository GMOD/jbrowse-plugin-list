...
export function genomeToMSA({ model }) {
    ...
    const assembly = assemblyManager.get(mafRegion?.assemblyName ?? connectedView.assemblyNames[0] ?? '');
    const canonical = (name) => (assembly?.initialized ? assembly.getCanonicalRefName(name) : undefined) ??
        name;
    const hoveredRefName = canonical(refName);
    if (mafRegion) {
        if (hoveredRefName !== canonical(mafRegion.refName) ||
