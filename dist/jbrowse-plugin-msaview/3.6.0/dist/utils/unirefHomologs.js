...
import { cleanGeneCandidate, dedupeLabels, defaultMaxSpecies, } from './ncbiOrthologs';
const UNIPROT = 'https://rest.uniprot.org';
export const unirefIdentities = [50, 90];
...
export const UNIPROT_ACCESSION = /^(?:[OPQ][0-9][A-Z0-9]{3}[0-9]|[A-NR-Z][0-9](?:[A-Z][A-Z0-9]{2}[0-9]){1,2})(?:-\d+)?$/;
const ENTRY_FIELDS = 'accession,id,reviewed,organism_name,organism_id,protein_name,sequence';
export function parseEntry(entry) {
    ...
    return {
        ...
        title: entry.proteinDescription?.recommendedName?.fullName?.value ??
            entry.proteinDescription?.submissionNames?.[0]?.fullName?.value,
        sequence: sequence.value,
    };
}
function parseEntries(json) {
    return (json.results ?? []).flatMap(e => {
 ...
 */
export async function resolveUniProtEntry(candidates, taxId, signal) {
    for (const raw of candidates) {
        ...
        if (UNIPROT_ACCESSION.test(candidate)) {
            const json = await jsonfetch(`${UNIPROT}/uniprotkb/${candidate.split('-')[0]}?fields=${ENTRY_FIELDS}`, { signal });
        ...
        const json = await jsonfetch(`${UNIPROT}/uniprotkb/search?query=${encodeURIComponent(query)}&fields=${ENTRY_FIELDS}&size=25`, { signal });
...
export async function fetchClusterId(accession, identity, signal) {
    const query = `(uniprot_id:${accession}) AND (identity:${identity / 100})`;
    const json = (await jsonfetch(`${UNIPROT}/uniref/search?query=${encodeURIComponent(query)}&fields=id,count&size=1`, { signal }));
 ...
 */
export async function fetchClusterMembers({ clusterId, identity, referenceProteomesOnly = true, taxa, exclude, limit = defaultMaxSpecies, onProgress, signal, }) {
    const query = [
        `uniref_cluster_${identity}:${clusterId}`,
        ...(referenceProteomesOnly ? [REFERENCE_PROTEOME] : []),
    ].join(' AND ');
    let url = `${UNIPROT}/uniprotkb/search?query=${encodeURIComponent(query)}&fields=${ENTRY_FIELDS}&size=${PAGE}&format=json`;
    ...
    while (url && scanned < MAX_SCANNED) {
        ...
        onProgress?.(`Read ${scanned}${total ? ` of ${total}` : ''} cluster members, ${bySpecies.size} species...`);
 ...
 */
export async function fetchUnirefHomologs({ candidates, taxId, identity = 50, referenceProteomesOnly = true, taxa, exclude, limit = defaultMaxSpecies, onProgress, signal, }) {
    onProgress('Resolving the query at UniProt...');
    const query = await resolveUniProtEntry(candidates, taxId, signal);
    if (!query) {
        throw new Error(`UniProt has no entry for ${candidates.join(', ')} in taxon ${taxId}. Try the BLAST tab, which needs no gene identifier.`);
    }
    ...
    if (members.length < 1) {
        throw new Error(`${cluster.id} has no other ${referenceProteomesOnly ? 'reference-proteome ' : ''}member to align ${query.id} against`);
    }
    const labels = dedupeLabels(members.map(m => m.commonName ?? m.scientificName));
    return {
        query,
        clusterId: cluster.id,
        total,
