...
import { UNIPROT_ACCESSION, fetchClusterMembers, parseEntry, resolveUniProtEntry, } from './unirefHomologs';
const entry = (accession, taxonId, extra = {}) => ({
...
test('parseEntry reads the reviewed flag off the entry type', () => {
    expect(parseEntry(entry('P04637', 9606, { entryType: 'UniProtKB reviewed (Swiss-Prot)' }))?.reviewed).toBe(true);
...
function stubFetch(pages) {
    ...
    vi.stubGlobal('fetch', (url) => {
        ...
        return Promise.resolve(new Response(JSON.stringify(page.body), { status: 200, headers }));
    });
    return calls;
}
...
test('fetchClusterMembers follows the Link header and asks for reference proteomes', async () => {
    ...
    expect(decodeURIComponent(calls[0])).toContain('uniref_cluster_90:UniRef90_X AND keyword:KW-1185');
...
test('resolveUniProtEntry fetches an accession directly and searches a symbol', async () => {
    ...
    expect((await resolveUniProtEntry(['P04637'], 9606))?.accession).toBe('P04637');
    expect(calls[0]).toContain('/uniprotkb/P04637?');
    expect((await resolveUniProtEntry(['gene:TP53'], 9606))?.accession).toBe('Q2');
    expect(decodeURIComponent(calls[1])).toContain('gene_exact:"TP53" AND organism_id:9606');
});
...
test('resolveUniProtEntry keeps an isoform suffix', async () => {
    ...
    expect((await resolveUniProtEntry(['P04637-2'], 9606))?.accession).toBe('P04637-2');
