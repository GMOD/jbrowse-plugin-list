...
import { cleanGeneCandidate, dedupeLabels, fetchOrthologGenes, fetchRepresentativeProteins, parseFasta, resolveGeneId, } from './ncbiOrthologs';
...
test('resolveGeneId searches each cleaned symbol once', async () => {
    ...
    expect(await resolveGeneId(['gene:TP53', 'TP53', ' TP53 ', 'NM_000546.6'], 9606)).toBeUndefined();
...
test('an abort stops a chunked lookup at the request in flight', async () => {
    ...
    const ids = Array.from({ length: 400 }, (_, i) => String(100000 + i));
    await expect(fetchRepresentativeProteins(ids, controller.signal)).rejects.toThrow(expect.objectContaining({ name: 'AbortError' }));
