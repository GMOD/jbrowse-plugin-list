...
function getCachedTaxonomies(taxids) {
    return bestEffort('taxonomy cache read', async () => {
...
function saveTaxonomyCache(entries) {
    return bestEffort('taxonomy cache write', async () => {
        const db = await getDB();
...
export async function fetchTaxonomyInfo(taxidsWithRepeats, signal) {
    ...
    for (let i = 0; i < uncachedTaxids.length; i += batchSize) {
        ...
        try {
            ...
            const text = await eutilsText(efetchUrl({ db: 'taxonomy', id: idsParam, retmode: 'xml' }), { signal });
