...
async function fetchSequences(accessions, signal) {
    const map = new Map();
    for (let i = 0; i < accessions.length; i += UNIPROT_CHUNK) {
        const chunk = accessions.slice(i, i + UNIPROT_CHUNK);
        const json = await jsonfetch(`${UNIPROT}/accessions?accessions=${chunk.join(',')}&fields=accession,sequence&format=json`, { signal });
 ...
 */
async function matchOrthologs(candidates, taxId, targets, signal) {
    let matched;
 ...
 */
export async function fetchPantherOrthologs({ candidates, taxId, taxa, exclude, limit = defaultMaxSpecies, onProgress, signal, }) {
    ...
    const [names, sequences] = await Promise.all([
        taxonomyNames(picks.map(p => p.genome.taxId), signal),
        fetchSequences([
 ...
 */
async function taxonomyNames(taxIds, signal) {
    try {
