...
function feature(type, subfeatures = ['CDS']) {
...
const legacyHost = (feat) => ({
    contextMenuItems: () => [],
    contextMenuFeature: feat,
});
async function fetched(target) {
...
describe('launchTarget', () => {
    ...
    test('falls back to a synchronous contextMenuFeature', () => {
        ...
        expect(target && 'feature' in target && target.feature.get('type')).toBe('mRNA');
    });
    ...
    test('declines a gene-like feature with no CDS below it', () => {
        expect(launchTarget(legacyHost(feature('lnc_RNA', ['exon'])))).toBeUndefined();
    });
