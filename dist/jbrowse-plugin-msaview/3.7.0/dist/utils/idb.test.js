...
const { opens } = vi.hoisted(() => ({ opens: [] }));
vi.mock('idb', () => ({
    openDB: (_name, _version, callbacks) => new Promise((resolve, reject) => {
...
describe('bestEffort', () => {
    ...
    test('answers the fallback when it throws', async () => {
        ...
        expect(await bestEffort('read', () => Promise.reject(new Error('quota')), 0)).toBe(0);
    });
});
