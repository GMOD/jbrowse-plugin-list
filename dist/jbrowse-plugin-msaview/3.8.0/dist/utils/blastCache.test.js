...
import { createCacheKey, getAllCachedResults, saveBlastResult, } from './blastCache';
...
describe('createCacheKey', () => {
    ...
    test('a different hit count is a different row', () => {
        expect(createCacheKey({ ...base, maxHits: 50 })).not.toBe(createCacheKey({ ...base, maxHits: 500 }));
    });
    // rows saved before the hit count was in the key must still resolve
    test('the default hit count keeps the key it always had', () => {
        expect(createCacheKey({ ...base, maxHits: 100 })).toBe('uniprotkb_swissprot:clustalo:MKV');
