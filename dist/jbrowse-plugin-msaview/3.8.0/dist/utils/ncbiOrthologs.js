...
export async function fetchOrthologGenes(geneId, { taxa, exclude, limit = defaultMaxSpecies, } = {}) {
    const json = await jsonfetch(`${DATASETS}/gene/id/${geneId}/orthologs?returned_content=COMPLETE`);
...
export async function fetchRepresentativeProteins(geneIds) {
    ...
    for (let i = 0; i < geneIds.length; i += PRODUCT_REPORT_CHUNK) {
        ...
        const json = await jsonfetch(`${DATASETS}/gene/id/${chunk.join(',')}/product_report?page_size=${chunk.length}`);
...
export async function fetchProteinForGene(geneId) {
    ...
    const seq = parseFasta(await eutilsText(efetchUrl({ db: 'protein', id: acc, rettype: 'fasta', retmode: 'text' }))).get(acc);
...
export async function fetchOrthologRows({ geneId, taxa, exclude, limit, onProgress, }) {
    ...
    const seqByAcc = parseFasta(await eutilsText(...efetchPost({
        db: 'protein',
        id: accessions.join(','),
        rettype: 'fasta',
        retmode: 'text',
