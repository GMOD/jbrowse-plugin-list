...
export function resolveConnectedTranscriptIfNeeded(self) {
    ...
    void (async () => {
        try {
            self.setProgress(`Looking up ${connectedTranscript} in the genome view...`);
            const transcript = await findTranscript(self, connectedTranscript);
            if (!transcript) {
                throw new Error(`Transcript "${connectedTranscript}" was not found in the ${trackCount} track(s) open in the connected genome view. Open the gene track that carries it, and make sure the view is on its locus.`);
            }
            const proteinSequence = await translate(self, transcript);
            if (!proteinSequence) {
                throw new Error(`Transcript "${connectedTranscript}" has no CDS to translate`);
            }
