...
export async function handleFetch(url, args) {
    ...
    try {
        ...
        if (e instanceof TypeError) {
            throw new Error(`Could not reach ${hostOf(url)} — the request failed at the network level. This is usually a CORS restriction (the server refused to let this site read the response), an offline connection, or a blocked request.`, { cause: e });
        }
        throw e;
    }
    if (!response.ok) {
