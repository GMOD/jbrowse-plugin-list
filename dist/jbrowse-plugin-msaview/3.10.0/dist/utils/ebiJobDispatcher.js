...
export function getEbiEmail() {
    const configured = readLocalStorage(EBI_EMAIL_STORAGE_KEY, DEFAULT_EBI_EMAIL).trim();
...
async function submitEbiJob({ tool, params, signal, }) {
        ...
        check: async () => {
            ...
            try {
                ...
                if (Date.now() - failingSince >= STATUS_FAILURE_BUDGET_MS) {
                    throw new Error(`Could not reach EBI to check ${tool} job ${jobId} for ${Math.round(STATUS_FAILURE_BUDGET_MS / 60_000)} minutes`, { cause: e });
                }
...
export async function runEbiJob({ tool, label, params, onProgress, onRid, signal, }) {
    ...
    return {
        jobId,
        result: (type) => textfetch(`${EBI_BASE}/${tool}/result/${jobId}/${type}`, { signal }),
    };
}
