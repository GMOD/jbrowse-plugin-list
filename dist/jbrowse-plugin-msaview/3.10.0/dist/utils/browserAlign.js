 ...
 */
export function alignToQuery(query, target) {
    ...
    const matched = Array.from({ length: n }).fill('-');
    const inserts = Array.from({ length: n + 1 }).fill('');
...
export function mergeOnQuery(query, aligned) {
    ...
    return [
        {
            name: query.name,
            sequence: rowOf(query.sequence.split(''), Array.from({ length: n + 1 }).fill('')),
        },
...
export function checkPairSize(name, length, queryLength) {
    if (length * queryLength > MAX_PAIR_CELLS) {
        throw new Error(`${name} (${length} residues) against the ${queryLength}-residue query is too large to align in the browser, which stops at ${MAX_PAIR_CELLS / 1e6}M residue pairs per sequence. Choose an EBI aligner such as Clustal Omega instead.`);
    }
}
/**
 ...
 */
export async function alignInBrowser({ query, targets, onProgress, signal, }) {
    ...
    for (const target of targets) {
        if (sinceYield >= CELLS_PER_YIELD) {
            onProgress?.(`Aligning ${aligned.length + 1} of ${targets.length} sequences in the browser...`);
...
export function parseFastaRecords(text) {
    const records = [];
