 ...
 */
export async function resolveTaxId(query, signal) {
 ...
 */
export async function resolveAssemblySpecies(assemblyName, signal) {
    const term = assemblyName.trim();
    if (!term) {
        return undefined;
    }
    const search = await eutilsJson(eutilsUrl('esearch', {
        db: 'assembly',
        term,
        retmax: '1',
        retmode: 'json',
    ...
    const summary = await eutilsJson(eutilsUrl('esummary', { db: 'assembly', id: uid, retmode: 'json' }), { signal });
