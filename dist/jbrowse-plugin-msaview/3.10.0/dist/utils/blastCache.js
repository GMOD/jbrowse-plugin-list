 ...
 * default.
 */
export function createCacheKey({ proteinSequence, blastDatabase, msaAlgorithm, searchProgram, transcriptId, maxHits, }) {
    const idPart = transcriptId ? `:${transcriptId}` : '';
    const hitsPart = maxHits === undefined || maxHits === defaultMaxHits ? '' : `:n${maxHits}`;
 ...
 * failed launch.
 */
export function saveBlastResult({ proteinSequence, blastDatabase, msaAlgorithm, searchProgram, maxHits, msa, tree, treeMetadata, rid, geneId, transcriptId, transcriptName, geneName, }) {
    return bestEffort('BLAST history write', async () => {
