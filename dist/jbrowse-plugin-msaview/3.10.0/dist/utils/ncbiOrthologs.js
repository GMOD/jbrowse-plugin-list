...
import { efetchPost, efetchUrl, eutilsJson, eutilsText, eutilsUrl, } from './eutils';
 ...
 * candidates.
 */
export function cleanGeneCandidate(raw) {
    return raw
        .trim()
        .replace(/^\w+:/, '')
        .replace(/\.\d+$/, '');
 ...
 */
export async function resolveGeneId(candidates, taxId, signal) {
    const asked = new Set();
    for (const raw of candidates) {
        ...
        asked.add(cleaned);
        const json = await eutilsJson(eutilsUrl('esearch', {
            db: 'gene',
 ...
 */
export async function fetchOrthologGenes(geneId, { taxa, exclude, limit = defaultMaxSpecies, signal, } = {}) {
    const json = await jsonfetch(`${DATASETS}/gene/id/${geneId}/orthologs?returned_content=COMPLETE`, { signal });
 ...
 */
export async function fetchRepresentativeProteins(geneIds, signal) {
    const byGene = new Map();
    for (let i = 0; i < geneIds.length; i += PRODUCT_REPORT_CHUNK) {
        const chunk = geneIds.slice(i, i + PRODUCT_REPORT_CHUNK);
        const json = await jsonfetch(`${DATASETS}/gene/id/${chunk.join(',')}/product_report?page_size=${chunk.length}`, { signal });
 ...
 */
export async function fetchProteinForGene(geneId, signal) {
    const acc = (await fetchRepresentativeProteins([geneId], signal)).get(geneId);
    if (!acc) {
        return undefined;
    }
    const seq = parseFasta(await eutilsText(efetchUrl({ db: 'protein', id: acc, rettype: 'fasta', retmode: 'text' }), { signal })).get(acc);
 ...
 */
export async function fetchOrthologRows({ geneId, taxa, exclude, limit, onProgress, signal, }) {
    ...
    const proteinByGene = await fetchRepresentativeProteins(genes.map(g => g.geneId), signal);
