...
import { generateDataStoreId, retrieveMsaData, storeMsaData, } from './msaDataStore';
...
function sharedRowStore(initial) {
    const rows = new Map([['shared', initial]]);
...
describe('restoring a view from IndexedDB', () => {
    ...
    test('a reset while the read is pending discards what it reads', async () => {
        let resolve = () => { };
        mockRetrieve.mockReturnValue(new Promise(r => {
            resolve = r;
        }));
        const model = view({ dataStoreId: 'msa-1' });
        model.reset();
...
describe('keeping IndexedDB up to date', () => {
    ...
    test('a large alignment is written under a fresh id', async () => {
        ...
        model.setMSA(BIG_MSA);
        await settle();
        expect(mockStore).toHaveBeenCalledWith(`${NEW_ID}-1`, expect.objectContaining({ msa: BIG_MSA }));
    ...
    test('an edit after the first write updates the row it wrote', async () => {
        ...
        await settle();
        expect(mockStore).toHaveBeenLastCalledWith(`${NEW_ID}-1`, expect.objectContaining({ msa: BIG_MSA, tree: BIG_TREE }));
    });
