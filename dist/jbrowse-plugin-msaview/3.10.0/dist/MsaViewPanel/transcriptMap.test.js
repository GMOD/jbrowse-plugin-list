...
import { MAX_CODING_BASES, proteinPositionsInRange, transcriptMap, } from './transcriptMap';
function cds(start, end, extra = {}) {
    return { type: 'CDS', start, end, ...extra };
}
function mapped(feature) {
...
describe('transcriptMap reads the CDS rows translation reads', () => {
    test('a lowercase cds row', () => {
        const map = mapped(transcript({ subfeatures: [{ ...cds(100, 103), type: 'cds' }] }));
        expect(map.codingPositions).toEqual([100, 101, 102]);
    });
...
describe('proteinPositionsInRange', () => {
    ...
    test('on the reverse strand the positions still come from g2p', () => {
        const reverse = mapped(transcript({ strand: -1, subfeatures: [cds(0, 3), cds(10_000, 10_003)] }));
