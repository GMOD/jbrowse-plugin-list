...
export function arrayInsertionKeys(array) {
    const ret = [];
 ...
 */
export function absorbAdjacentInsertions(reads, arrays) {
    ...
    return ordered.map((array, i) => {
        ...
        const ceiling = Math.min(array.end + margin, (ordered[i + 1]?.start ?? Infinity) - 1);
        let { start, end } = array;
