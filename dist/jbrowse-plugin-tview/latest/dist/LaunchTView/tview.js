...
export function parseRead(feature, name) {
 ...
 */
export function alignInsertionColumns(reads) {
    const byPos = new Map();
    ...
    const gapped = new Map([...byPos].map(([pos, seqs]) => [pos, alignInsertions(seqs)]));
    return reads.map(read => ({
        ...read,
...
export function maxInsertionWidths(reads) {
    const ret = new Map();
    for (const read of reads) {
        for (const [pos, ins] of read.insertions) {
 ...
 */
export function buildColumnLayout(start, end, insWidths) {
    const offsets = [0];
    let col = 0;
...
function getReadNames(features) {
 ...
 */
export function planTviewMsa({ features, refName, start, end, }) {
    const names = getReadNames(features);
    const reads = alignInsertionColumns(clipInsertions(features.map((f, i) => parseRead(f, names[i])).sort(byStart), start, end));
