...
import { matchSampleId, parseAssemblyAndChr, parseAssemblyAndChrSimple, selectReferenceSequenceString, } from './parseAssemblyName';
...
describe('selectReferenceSequenceString', () => {
    ...
    test('uses refAssemblyName when provided and exists', () => {
        const result = selectReferenceSequenceString(alignments, 'mm10', 'hg38', 'panTro6');
    ...
    test('falls back to queryAssemblyName when refAssemblyName is undefined', () => {
        const result = selectReferenceSequenceString(alignments, undefined, 'hg38', 'panTro6');
        expect(result).toBe(hg38Seq);
    });
    test('falls back to firstAssemblyNameFound when queryAssemblyName does not match', () => {
        const result = selectReferenceSequenceString(alignments, undefined, 'galGal6', // not in alignments
        'hg38');
        expect(result).toBe(hg38Seq);
    });
    ...
    test('returns undefined when refAssemblyName does not exist in alignments', () => {
        const result = selectReferenceSequenceString(alignments, 'nonexistent', undefined, undefined);
        expect(result).toBeUndefined();
    });
    test('returns undefined when no matches and all params undefined', () => {
        const result = selectReferenceSequenceString(alignments, undefined, undefined, undefined);
        expect(result).toBeUndefined();
    });
    ...
    test('skips refAssemblyName when it does not exist and uses queryAssemblyName', () => {
        const result = selectReferenceSequenceString(alignments, 'galGal6', // not in alignments
        'hg38', 'panTro6');
        expect(result).toBe(hg38Seq);
    });
...
describe('real-world MAF format parsing', () => {
    ...
    test('multiple assemblies from same MAF block produce correct lookup', () => {
        ...
        expect(selectReferenceSequenceString(alignments, '', 'ce10', 'ce10')).toBe(ce10Seq);
        // When refAssemblyName is configured to override
        expect(selectReferenceSequenceString(alignments, 'caePb3', 'ce10', 'ce10')).toBe(caePb3Seq);
    });
});
