...
export function decodeBase(encoded, index) {
...
export function decodeBaseLower(encoded, index) {
    if (index < 0 || index >= encoded.length) {
        return '';
    }
    const byteIndex = index >> 1;
...
export function getBaseCode(encoded, index) {
    if (index < 0 || index >= encoded.length) {
        return 12;
    }
    const byteIndex = index >> 1;
...
export function decodeSequenceLower(encoded) {
    let result = '';
    for (let i = 0; i < encoded.length; i++) {
...
//# sourceMappingURL=sequenceEncoding.js.map