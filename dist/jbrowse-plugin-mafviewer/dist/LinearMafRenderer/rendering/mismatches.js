        ...
        if (refChar !== '-') {
            if (alignChar !== '-') {
                ...
                if (refChar.toLowerCase() !== base && alignChar !== ' ') {
                    // Mismatch
                    fillRect(ctx, xPos, rowTop, scale + GAP_STROKE_OFFSET, h, canvasWidth, mismatchRendering ? (colorForBase[base] ?? 'black') : 'orange');
                    if (shouldAddToSpatialIndex(xPos, rowIndex, context)) {
                        addToSpatialIndex(context, xPos, rowTop, xPos + context.scale + GAP_STROKE_OFFSET, rowTop + context.h, rowIndex, { pos: genomicOffset + alignmentStart, chr, base, rowIndex });
                    }
                }
                else if (showAllLetters) {
                    // Match (when showing all letters)
                    fillRect(ctx, xPos, rowTop, scale + GAP_STROKE_OFFSET, h, canvasWidth, mismatchRendering ? (colorForBase[base] ?? 'black') : 'lightblue');
                    if (shouldAddToSpatialIndex(xPos, rowIndex, context)) {
                        addToSpatialIndex(context, xPos, rowTop, xPos + context.scale + GAP_STROKE_OFFSET, rowTop + context.h, rowIndex, { pos: genomicOffset + alignmentStart, chr, base, rowIndex });
                    }
                }
