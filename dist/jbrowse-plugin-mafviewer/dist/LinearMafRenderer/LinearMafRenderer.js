...
import { finalizeRendering, initRenderingContext, renderFeature, } from './makeImageData';
...
export default class LinearMafRenderer extends FeatureRendererType {
    ...
    async render(renderProps) {
        const { regions, bpPerPx, samples, rowHeight, sessionId, adapterConfig, highResolutionScaling = 1, } = renderProps;
        const region = regions[0];
        const height = samples.length * rowHeight + 100;
        const width = (region.end - region.start) / bpPerPx;
        // Create canvas with high resolution scaling support
        const canvas = createCanvas(Math.ceil(width * highResolutionScaling), Math.ceil(height * highResolutionScaling));
        ...
        const { renderingContext, sampleToRowMap, region: expandedRegion, } = initRenderingContext(ctx, renderProps);
        ...
        await subscribeToObservable(adapter.getFeatures(queryRegion, renderProps), (feature) => {
            if (this.featurePassesFilters(renderProps, feature)) {
                renderFeature(feature, expandedRegion, bpPerPx, sampleToRowMap, renderingContext);
            }
        });
...
//# sourceMappingURL=LinearMafRenderer.js.map