...
describe('BgzipTaffyAdapter methods', () => {
    ...
    function createTestAdapter() {
        ...
        return {
            parseBases(basesStr, runLengthEncodeBases) {
            ...
            parseCoordinatesAndEstablishBlock(pBlock, instructions) {
                const block = {
                    rows: [],
                    columnNumber: 0,
                };
    ...
    test('parseCoordinatesAndEstablishBlock creates new block from scratch', () => {
        const adapter = createTestAdapter();
        const instructions = filterFirstLineInstructions(parseRowInstructions('s 0 ce10.chrI 100 + 1000 s 1 mm10.chr1 200 + 2000'));
        const block = adapter.parseCoordinatesAndEstablishBlock(undefined, instructions);
    ...
    test('parseCoordinatesAndEstablishBlock copies from previous block', () => {
        const adapter = createTestAdapter();
        // First block
        const instructions1 = parseRowInstructions('i 0 ce10.chrI 100 + 1000 i 1 mm10.chr1 200 + 2000');
        const block1 = adapter.parseCoordinatesAndEstablishBlock(undefined, instructions1);
    ...
    test('parseCoordinatesAndEstablishBlock handles insert in middle of block', () => {
        ...
        const block2 = adapter.parseCoordinatesAndEstablishBlock(block1, instructions2);
    ...
    test('parseCoordinatesAndEstablishBlock handles delete', () => {
        ...
        const instructions1 = parseRowInstructions('i 0 ce10.chrI 100 + 1000 i 1 mm10.chr1 200 + 2000 i 2 rn6.chr1 300 + 3000');
        const block1 = adapter.parseCoordinatesAndEstablishBlock(undefined, instructions1);
...
describe('BgzipTaffyAdapter integration tests', () => {
    ...
    test('adapter handles single-entry index files (evolverMammals)', async () => {
        const adapter = new BgzipTaffyAdapter(configSchema.create({
            ...
            taiLocation: {
                localPath: require.resolve('../../test_data/evolverMammals.taf.gz.tai'),
                locationType: 'LocalPathLocation',
            },
        }));
        const refNames = await adapter.getRefNames();
...
//# sourceMappingURL=BgzipTaffyAdapter.test.js.map