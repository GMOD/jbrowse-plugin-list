    ...
    *parseTafBlocksStreaming(buffer, runLengthEncodeBases) {
        ...
        for (const line of lines) {
            ...
            if (hasCoordinates) {
                ...
                const basesOnly = basesAtIndex !== -1
    ...
    setupPre() {
        if (!this.setupP) {
            this.setupP = this.doSetup().catch((e) => {
    ...
    getFeatures(query, opts) {
        ...
        return ObservableCreate(async (observer) => {
            try {
                ...
                const readLength = endBlock > startBlock
                    ? endBlock - startBlock + MIN_BLOCK_SIZE
                    : MIN_BLOCK_SIZE;
                const response = await updateStatus('Downloading alignments', statusCallback, () => file.read(readLength, startBlock));
                const buffer = await unzip(response);
                const startOffset = firstEntry.virtualOffset.dataPosition;
                const nextOffset = nextEntry.virtualOffset.dataPosition;
                const endOffset = endBlock === startBlock && nextOffset > startOffset
                    ? nextOffset
                    : buffer.length;
