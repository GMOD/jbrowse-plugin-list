...
describe('parseRowInstructions', () => {
    ...
    test('parses multiple insert instructions', () => {
        const result = parseRowInstructions('i 0 hg38.chr1 1000 + 248956422 i 1 mm10.chr1 500 + 195471971');
    ...
    test('parses complex line with inserts and gaps', () => {
        const result = parseRowInstructions('i 3 mr.mrrefChr1 178357 + 182340 g 6 6 i 7 simMouse_chr6.simMouse.chr6 630720 + 636262 i 8 simRat_chr6.simRat.chr6 642233 + 647215');
...
describe('filterFirstLineInstructions', () => {
    test('converts s instructions to i instructions', () => {
        const instructions = parseRowInstructions('s 0 ce10.chrI 2272337 + 15072423 s 1 caeSp111.Scaffold80 35303 - 57550');
    ...
    test('removes d instructions', () => {
        const instructions = parseRowInstructions('d 2 d 2 s 0 ce10.chrI 100 + 1000 s 1 mm10.chr1 200 + 2000');
        const filtered = filterFirstLineInstructions(instructions);
        expect(filtered).toHaveLength(2);
        expect(filtered.every(ins => ins.type === 'i')).toBe(true);
    });
    test('removes g instructions', () => {
        const instructions = parseRowInstructions('g 0 50 s 0 ce10.chrI 100 + 1000 g 1 100');
        const filtered = filterFirstLineInstructions(instructions);
        expect(filtered).toHaveLength(1);
        expect(filtered[0]).toMatchObject({ type: 'i', row: 0 });
    });
    test('removes G instructions', () => {
        const instructions = parseRowInstructions('G 0 ACGT s 0 ce10.chrI 100 + 1000');
        const filtered = filterFirstLineInstructions(instructions);
        expect(filtered).toHaveLength(1);
        expect(filtered[0]).toMatchObject({ type: 'i', row: 0 });
    });
    test('preserves i instructions unchanged', () => {
        const instructions = parseRowInstructions('i 0 ce10.chrI 100 + 1000 i 1 mm10.chr1 200 + 2000');
        const filtered = filterFirstLineInstructions(instructions);
        expect(filtered).toHaveLength(2);
        expect(filtered[0]).toMatchObject({ type: 'i', row: 0 });
    ...
    test('handles real TAF indexed line with d and s instructions', () => {
        ...
        const instructions = parseRowInstructions('d 2 d 2 s 0 ce10.chrI 2272337 + 15072423 s 1 caeSp111.Scaffold80 35303 - 57550');
        const filtered = filterFirstLineInstructions(instructions);
...
//# sourceMappingURL=rowInstructions.test.js.map