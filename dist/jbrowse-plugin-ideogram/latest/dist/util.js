...
export async function openReactomeView(pwId, pathways, pwName, geneName, model) {
    const session = getSession(model);
    const rv = session.views.find(view => view.type === 'ReactomeView');
    if (rv) {
        rv.setPathways(pathways);
        rv.setSelectedPathway(pwId);
        rv.setGene(geneName);
        rv.setMessage(`Pathways relating to ${geneName} are being displayed. "${pwName}" has been selected.`);
    }
    else {
        ...
        view.setMessage(`Pathways relating to ${geneName} are being displayed. "${pwName}" has been selected.`);
        await when(() => view.initialized);
    }
}
export async function navToAnnotation(locString, model) {
    // @ts-ignore
    const { assembly } = model.view;
    const session = getSession(model);
    const lgv = session.views.find(view => view.type === 'LinearGenomeView' &&
        // @ts-ignore
    ...
    if (lgv) {
        ...
        if (sessionAssembly) {
            try {
                const loc = parseLocString(locString, refName => session.assemblyManager.isValidRefName(refName, assembly));
                const { refName } = loc;
...
export async function populateAnnotations(model) {
    const session = getSession(model);
    if (model.annotationsLocation) {
        model.setShowLoading(true);
        const { widget, ideo, pathways, res } = await generateAnnotations(model.annotationsLocation, model.withReactome);
        if (res.type !== 2) {
            model.setWidgetAnnotations(widget);
            model.setIdeoAnnotations(ideo);
        }
        if (model.withReactome) {
            const tview = session.views.find((view) => (view === null || view === void 0 ? void 0 : view.isAnalysisResults) && view.ideogramId === model.ideogramId);
            if (tview) {
