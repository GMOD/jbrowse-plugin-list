...
export async function applyColorTheme({ plugin, colorScheme, structures, }) {
    for (const { molstarStructure, entityId } of structures) {
        const ref = plugin.managers.structure.hierarchy.findStructure(molstarStructure);
        if (ref) {
            ...
            const theme = colorScheme === 'mapped-chain'
                ? { color: colorScheme, colorParams: { entityId: entityId ?? '' } }
                : { color: colorScheme };
            await plugin.managers.structure.component.updateRepresentationsTheme(ref.components, theme);
        }
    }
}
