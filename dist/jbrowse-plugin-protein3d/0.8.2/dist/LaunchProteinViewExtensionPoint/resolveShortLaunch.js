...
import { getTranscriptFeatures, stripTrailingVersion, } from '../LaunchProteinView/utils/util';
...
function transcriptMatches(transcript, transcriptId) {
    ...
    return [transcript.get('name'), transcript.get('id'), transcript.id()].some(candidate => typeof candidate === 'string' &&
 ...
 */
export async function resolveShortLaunch({ session, structureUrl, transcriptId, connectedView, }) {
    if (!transcriptId) {
    ...
    if (!transcript) {
        throw new Error(`transcript "${transcriptId}" not found at ${loc} in tracks [${trackIds.join(', ')}]`);
    }
    const hasCds = (transcript.get('subfeatures') ?? []).some((sub) => sub.get('type') === 'CDS');
    if (!hasCds) {
