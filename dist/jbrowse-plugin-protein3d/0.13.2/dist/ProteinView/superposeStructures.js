 ...
 */
export async function superposeStructures(plugin, loads) {
    const molstar = await loadMolstar();
    const { QueryContext, StateTransforms, StructureElement, StructureSelection, StructureSelectionQueries, PluginCommands, tmAlign, } = molstar;
    ...
    const traces = loads.flatMap(load => {
        ...
        const loci = StructureSelection.toLociWithSourceUnits(query(new QueryContext(first)));
    ...
    const pivotTransform = plugin.state.data.selectQ(q => q
    ...
    for (const { cells, loci } of mobile) {
        const { bTransform, tmScoreA, tmScoreB, rmsd, alignedLength } = tmAlign(pivot.loci, loci);
