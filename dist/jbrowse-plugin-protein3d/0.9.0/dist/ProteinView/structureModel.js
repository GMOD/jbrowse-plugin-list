...
import { chooseMappedEntity, interactionMatchesMappedEntity, } from './chooseMappedEntity';
...
const Structure = types
    ...
    get confidenceCells() {
        ...
        return c && c.entityId === this.mappedEntityId && looksLikePlddt(c.values)
            ? mapResidueValuesToColumns(c.values, this.structurePositionToAlignmentMap)
            : [];
    },
    /**
     * #getter
    ...
    afterAttach() {
        ...
        const forMappedEntity = (info) => {
            if (!info ||
                !interactionMatchesMappedEntity(info.entityId, self.coordinateMapper ? self.mappedEntityId : undefined)) {
                return undefined;
            }
