...
const ungapped = (row) => row.replaceAll('-', '').toUpperCase();
/**
 ...
 */
export function pairwiseAlignmentSequenceProblem(pa, transcript, structure) {
...
export function mappedStructureIdentity(pa) {
    const transcript = transcriptAlignedSeq(pa);
    const structure = structureAlignedSeq(pa);
    ...
    for (let i = 0, j = 0; i < structure.length; i++) {
        if (structure[i] !== '-') {
            if (transcript[i] !== '-') {
                mapped.set(j, transcript[i].toUpperCase() === structure[i].toUpperCase());
            }
            j++;
        }
    }
 ...
 * changes.
 */
export function unmapStructurePositions(pa, positions) {
    const transcript = transcriptAlignedSeq(pa);
    const structure = structureAlignedSeq(pa);
    const t = [];
    const s = [];
