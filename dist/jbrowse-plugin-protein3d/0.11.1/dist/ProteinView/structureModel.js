...
import { alignTranscriptToEntity, chooseMappedEntity, interactionMatchesMappedEntity, } from './chooseMappedEntity';
...
import { alignmentCol, makeCoordinateMapper, structurePos, transcriptRangeToStructureRange, } from './coordinates';
import { looksLikePlddt } from './extractPerResidueConfidence';
import { entityLabel, makeLabelSeqIdIndex, rangeToLabelSeqIds, residueNumber, residueRangeToPositions, toLabelSeqIds, } from './extractStructureSequences';
import { fetchUniProtStructureMappings, fusionPartnerPositions, } from './pdbUniProtMapping';
...
import { getPdbIdFromUrl, getUniprotIdFromAlphaFoldTarget, resolveStructureUrl, structureDisplayLabel, } from '../LaunchProteinView/utils/structureUrls';
import { stripStopCodon } from '../LaunchProteinView/utils/util';
import { alignmentLength, codonGenomeSpan, genomeToTranscriptSeqMapping, mappedStructureIdentity, unmapStructurePositions, } from '../mappings';
const Structure = types
    ...
    afterAttach() {
        ...
        if (initialResidues) {
            addDisposer(self, when(() => !!self.entities &&
                (self.mappedEntityId !== undefined ||
                    !self.userProvidedTranscriptSequence), () => {
                self.setClickedStructureRange(residueRangeToPositions(self.mappedEntity, initialResidues));
            }));
        }
        ...
        if (initialTranscriptResidues) {
            addDisposer(self, when(() => !!self.coordinateMapper && !self.loading, () => {
                self.setClickedStructureRange(transcriptRangeToStructureRange(self.coordinateMapper, initialTranscriptResidues));
            }));
        }
        ...
        addDisposer(self, autorun(() => {
            try {
                const { userProvidedTranscriptSequence, entities, alignmentAlgorithm, } = self;
