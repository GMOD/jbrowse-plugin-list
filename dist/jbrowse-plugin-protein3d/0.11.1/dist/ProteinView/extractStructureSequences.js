...
function canonicalSequencesByEntity(model) {
    const source = model.obj?.data.sourceData?.data;
 ...
 */
export function oneLetterSequence(codes, canonical) {
    const letters = Array.from(codes);
    return letters
        .map((c, i) => c === 'X' && canonical?.length === letters.length ? canonical[i] : c)
        .join('');
}
export function extractEntities(model) {
    ...
    return model.obj?.data.sequence.sequences.map(s => {
        ...
        return {
            entityId: s.entityId,
            seq: oneLetterSequence(s.sequence.code.toArray(), canonical.get(s.entityId)),
            seqIds,
