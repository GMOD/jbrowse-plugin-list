...
export function alignmentTooLarge(len1, len2) {
    return (len1 + 1) * (len2 + 1) > MAX_ALIGNMENT_CELLS;
}
function assertAlignable(len1, len2) {
    if (alignmentTooLarge(len1, len2)) {
        throw new Error(`Sequences too long to align (${len1} x ${len2} residues exceeds the ${MAX_ALIGNMENT_CELLS} cell limit)`);
    }
}
...
const IX_SHIFT = 2;
const IY_SHIFT = 4;
const readPointer = (trace, at, shift) => ((trace[at] >> shift) & 0b11);
/**
 ...
 */
function fillMatrices(seq1, seq2, gapOpen, gapExtend, local) {
    const m = seq1.length;
    const n = seq2.length;
    assertAlignable(m, n);
    const width = n + 1;
    ...
    for (let i = 1; i <= m; i++) {
        ...
        for (let j = 1; j <= n; j++) {
            const col = cols[j - 1];
            const matchScore = scoreRow < 0 || col < 0
                ? UNKNOWN_PAIR_SCORE
            ...
            const diagFrom = bestDiag === dM ? FROM_M : bestDiag === dIx ? FROM_IX : FROM_IY;
    ...
    return {
        ...
        startI: m,
        startJ: n,
        startMatrix: score === finalM ? FROM_M : score === finalIx ? FROM_IX : FROM_IY,
    };
}
/**
 ...
 */
function traceback(seq1, seq2, filled) {
...
export function needlemanWunsch(seq1, seq2, gapOpen = GAP_OPEN, gapExtend = GAP_EXTEND) {
    return traceback(seq1, seq2, fillMatrices(seq1, seq2, gapOpen, gapExtend, false));
}
/**
 * Smith-Waterman local alignment algorithm
 * Finds the best local alignment between subsequences
 */
export function smithWaterman(seq1, seq2, gapOpen = GAP_OPEN, gapExtend = GAP_EXTEND) {
    return traceback(seq1, seq2, fillMatrices(seq1, seq2, gapOpen, gapExtend, true));
}
...
export function scoredAlignment(seq1, seq2, algorithm) {
    const { alignedSeq1, alignedSeq2, score } = algorithm === 'smith_waterman'
