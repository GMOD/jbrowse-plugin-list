...
import { Button, Checkbox, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, FormControlLabel, Radio, RadioGroup, TextField, Typography, } from '@mui/material';
import { observer } from 'mobx-react';
import { STRUCTURE_FILE_ACCEPT, readStructureFile, } from '../../LaunchProteinView/utils/readStructureFile';
    ...
    const handleAdd = async () => {
        try {
            ...
            if (source) {
                ...
                const mapping = mapToTranscript && primary?.userProvidedTranscriptSequence
                    ? {
                        userProvidedTranscriptSequence: primary.userProvidedTranscriptSequence,
        ...
        React.createElement(DialogContent, null,
            ...
            choice === 'file' ? (React.createElement("div", { style: { marginBottom: 16 } },
                React.createElement(Button, { variant: "outlined", component: "label" },
                    ...
                    React.createElement("input", { type: "file", hidden: true, accept: STRUCTURE_FILE_ACCEPT, onChange: ({ target }) => {
                ...
                file ? (React.createElement(Typography, { variant: "body2", sx: { mt: 1 } },
                    "Selected: ",
                    file.name)) : null)) : null,
            canMap ? (React.createElement(FormControlLabel, { control: React.createElement(Checkbox, { checked: mapToTranscript, onChange: event => {
                        setMapToTranscript(event.target.checked);
                    } }), label: "Map to the same transcript as the first structure (alignment, hover and click linked to the genome)" })) : null,
