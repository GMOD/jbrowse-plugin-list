...
export function needlemanWunsch(seq1, seq2, gapOpen = GAP_OPEN, gapExtend = GAP_EXTEND) {
    ...
    const n = seq2.length;
    // Score matrices
    const M = []; // match/mismatch
    const Ix = []; // gap in seq2 (insertion in seq1)
    const Iy = []; // gap in seq1 (insertion in seq2)
    // Initialize matrices
    for (let i = 0; i <= m; i++) {
        M[i] = [];
        Ix[i] = [];
...
export function smithWaterman(seq1, seq2, gapOpen = GAP_OPEN, gapExtend = GAP_EXTEND) {
    ...
    for (let i = 1; i <= m; i++) {
        for (let j = 1; j <= n; j++) {
            ...
            M[i][j] = Math.max(0, M[i - 1][j - 1] + matchScore, Ix[i - 1][j - 1] + matchScore, Iy[i - 1][j - 1] + matchScore);
            // Ix[i][j]: best score ending with gap in seq2
            Ix[i][j] = Math.max(M[i - 1][j] + gapOpen, Ix[i - 1][j] + gapExtend);
            // Iy[i][j]: best score ending with gap in seq1
            Iy[i][j] = Math.max(M[i][j - 1] + gapOpen, Iy[i][j - 1] + gapExtend);
    ...
    let currentMatrix = M[i][j] >= Ix[i][j] && M[i][j] >= Iy[i][j]
        ? 'M'
        : Ix[i][j] >= Iy[i][j]
            ? 'Ix'
            : 'Iy';
