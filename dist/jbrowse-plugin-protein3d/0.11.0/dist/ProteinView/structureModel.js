...
import { getUniprotIdFromAlphaFoldTarget, resolveStructureUrl, structureDisplayLabel, } from '../LaunchProteinView/utils/structureUrls';
import { stripStopCodon } from '../LaunchProteinView/utils/util';
import { alignmentLength, codonGenomeSpan, genomeToTranscriptSeqMapping, } from '../mappings';
import { makeLabelSeqIdIndex, rangeToLabelSeqIds, residueNumber, residueRangeToPositions, toLabelSeqIds, } from './extractStructureSequences';
const Structure = types
     ...
     */
    setGenomeHoveredPosition(structureSeqPos) {
        self.hoverPosition =
    ...
    afterAttach() {
        ...
        addDisposer(self, autorun(() => {
            ...
            const transcriptPos = connectedView?.initialized
                ...
                : undefined;
            if (transcriptPos !== undefined) {
                self.setGenomeHoveredPosition(transcriptSeqToStructureSeqPosition?.[transcriptPos]);
            }
        ...
        const forMappedEntity = (info) => {
            if (!info ||
                !interactionMatchesMappedEntity(info.entityId, self.coordinateMapper ? self.mappedEntity?.entityId : undefined)) {
                return undefined;
            }
        ...
        const driveChannel = (channel, getLabelSeqIds) => {
            addDisposer(self, autorun(async () => {
