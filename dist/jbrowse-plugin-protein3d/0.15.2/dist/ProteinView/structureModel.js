...
const Structure = types
    ...
    setConnectedHoveredPosition(structureSeqPos, source = 'genome') {
        const next = structureSeqPos === undefined ? undefined : { structureSeqPos, source };
        const current = self.hoverPosition;
        if (next?.structureSeqPos !== current?.structureSeqPos ||
    ...
    setAligning(aligning, pendingEntityId) {
    ...
    return {
        actions: {
            ...
            alignInWorker({ name, args, inPlace, apply, fail, pendingEntityId, }) {
     ...
     */
    applyEntitySelection(selection) {
        const entities = self.entities ?? [];
        if (!selection) {
            const proteins = entities.filter(e => !e.nucleicAcid && e.seq.length > 0);
            self.setAlignmentSkipped(proteins.length
                ? `No chain could be aligned: ${proteins
                    .map(e => entityLabel(e))
                    .join(', ')} exceed the alignment size limit against this ${stripStopCodon(self.userProvidedTranscriptSequence).length} aa transcript`
     ...
     * #action
     */
    applyChosenEntity(entityId, alignment) {
    ...
    chooseEntity(entityId) {
        ...
        const align = () => alignTranscriptToEntity(transcript, entity.seq, self.alignmentAlgorithm);
        const apply = (scored) => {
            if (scored) {
                self.applyChosenEntity(entityId, scored.alignment);
            }
            else {
                self.parentView.setError(new Error(`${entity.chains.join('/') || entity.entityId} is too long to align to this transcript`));
                self.redecideAlignment();
            }
        };
    ...
    afterAttach() {
        ...
        addDisposer(self, autorun(() => {
            try {
                const { userProvidedTranscriptSequence: transcript, entities, alignmentAlgorithm: algorithm, pairwiseAlignment, } = self;
                ...
                if (pairwiseAlignment) {
                    ...
                    const fit = entityAlignedTo(pairwiseAlignment, transcript, entities, self.mappedEntityId);
                    if ('entityId' in fit) {
                    ...
                    const realign = (seq) => () => alignTranscriptToEntity(transcript, seq, algorithm);
                ...
                const choose = () => chooseMappedEntity(transcript, entities, algorithm);
                if (entities.some(e => !e.nucleicAcid && isIdentical(transcript, e.seq))) {
                    self.supersedeAlignment();
        ...
        addDisposer(self, autorun(() => {
            ...
            const hover = connectedHoverTranscriptPos({
                ...
                naming: assemblyNaming(assemblyManager, self.connectedView?.assemblyNames[0]),
            });
            if (hover) {
