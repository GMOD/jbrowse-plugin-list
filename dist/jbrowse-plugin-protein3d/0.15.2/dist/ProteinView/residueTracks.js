...
export function kyteDoolittle(aa) {
...
export function bandColor(bands, value) {
...
const HYDROPHILIC_RGB = [51, 102, 204];
const NEUTRAL_RGB = [247, 247, 247];
const HYDROPHOBIC_RGB = [230, 140, 40];
/**
 ...
 */
export function hydrophobicityRgb(score) {
    const t = Math.max(-1, Math.min(1, score / KYTE_DOOLITTLE_MAX));
    const end = t < 0 ? HYDROPHILIC_RGB : HYDROPHOBIC_RGB;
    const mix = (i) => Math.round(NEUTRAL_RGB[i] + (end[i] - NEUTRAL_RGB[i]) * Math.abs(t));
