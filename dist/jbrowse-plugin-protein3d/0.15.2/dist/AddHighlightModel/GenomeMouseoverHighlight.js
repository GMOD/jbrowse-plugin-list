...
const GenomeMouseoverHighlight = observer(function GenomeMouseoverHighlight({ model, }) {
    ...
    const connected = getStructuresConnectedTo(getProteinViews(session), id).length > 0;
    if (connected &&
        checkHovered(hovered) &&
        assemblyNaming(assemblyManager, assemblyNames[0]).isAssembly(hovered.hoverPosition.assemblyName)) {
