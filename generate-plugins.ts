  ...
  return [
    {
      ...
      // manifest: it is a pure function of packageName + bundle path, so
      // passing it down would only create a second place for it to be stale.
      latestUrl: latestRehostedUrl(plugin.packageName, bundleOf(plugin).path),
    },
  ]
})
